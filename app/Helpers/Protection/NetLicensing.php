...
class NetLicensing
{
    ...
    public function createShopToken(User $user, $successUrl = null, $cancelUrl = null, $successUrlTitle = null, $cancelUrlTitle = null)
    {
        ...
        if ($successUrl) {
            $query->where('success_url', $successUrl);
        }

        if ($cancelUrl) {
            $query->where('cancel_url', $cancelUrl);
        }

        if ($successUrlTitle) {
            $query->where('success_url_title', $successUrlTitle);
        }

        if ($cancelUrlTitle) {
            $query->where('cancel_url_title', $cancelUrlTitle);
        }
        ...
        if (! $protectionShopToken) {
            ...
            if ($successUrlTitle) {
                $token->setSuccessURLTitle($successUrlTitle);
            }

            if ($cancelUrlTitle) {
                $token->setCancelURLTitle($cancelUrlTitle);
            }
